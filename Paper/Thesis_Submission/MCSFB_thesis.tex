...
\author{\IEEEauthorblockN{{ Shuni Li \\ \medskip Joint work with David I Shuman and Yan Jin}}
...
In Sections \ref{Se:fb_design} and \ref{Se:partition}, we present the filter bank architecture and a method to choose the downsampling sets for graphs that are small enough to perform a full eigendecomposition of the graph Laplacian matrix (say 10,000 or fewer vertices). Our main contributions in this extended version of the work include developing a fast version of the proposed transform that scales efficiently for large, sparse graphs; i.e., one that does not require a full eigendecomposition of the graph Laplacian. Specifically, we design the filter bank to be more amenable to polynomial approximation (Section \ref{Se:filt_design}), and then estimate the number of samples required for each band, perform non-uniform random sampling for the filtered signals on each band, and use efficient convex optimization methods to reconstruct the interpolation for each band on the synthesis side (Section \ref{Se:samp_rec}). We then 
%{\color{red} discuss properties of the proposed fast filter bank transform (Section \ref{Se:fast_prop}), and } 
empirically explore the joint vertex-frequency localization of the dictionary atoms, the sparsity of the analysis coefficients, and the reconstruction error resulting from the numerical approximations (Section \ref{Se:ill2}).
...
Globally smooth signals trivially lead to sparse analysis coefficients because the coefficients are only nonzero for first set of vertices in the partition. More generally, signals that are concentrated in the graph Fourier domain necessarily have sparse M-CSFB analysis coefficients, because the coefficients for any channel whose filter does not overlap with the support of the signal in the graph Fourier domain are all equal to zero. %Similarly, if the spectral coefficients decay, the transform coefficients should decay.
%{\color{red}
%\begin{itemize}
...
\end{minipage}
\begin{minipage}[m]{0.16\linewidth}
\centerline{\small{Error (Vertex)}}\end{minipage}
\begin{minipage}[m]{0.16\linewidth}
\centerline{\small{Error (Spectral)}}\end{minipage}
\begin{minipage}[m]{0.16\linewidth}
\centerline{\small{MSE vs. \# Samples}}\end{minipage} \\
\begin{minipage}[m]{0.16\linewidth}
...
The fifth figure in each row shows that the reconstruction error decreases for all choices of the penalty filter as the number of samples increases.}\label{Fig:samp_tradeoff}
\end{figure*}
%%%%%%%
...
\end{align}
which can be solved, for example, with the preconditioned conjugate gradient method. 

%{\color{red}Add more details? Preconditioner? How to compute $\varphi_m(\L){\bf z}$.}
%
%%\begin{itemize}
...
%{\color{red}
%\section{Properties of the Fast M-CSFB Transform} \label{Se:fast_prop}
...
%}
%{\color{blue}
First, we explore the tradeoff between the number of samples and the reconstruction error. Note that the polynomial approximated filters have wider supports as compared to the ideal filters. By performing critical sampling based on the estimated supports of the ideal filters, we may not have enough samples to reconstruct any signal from the (wider) filtered subspace. %miss some parts of the spectrum on which the polynomial filters are supported. 
In order to get a better reconstruction, we might need to include more samples for each band. In Fig. \ref{Fig:samp_tradeoff}, we demonstrate that the reconstruction errors decrease as we increase the number of samples for each band. %, we get smaller reconstruction errors.
%}



Next, we examine average temperatures for March 2018, taken from the Gridded 5km GHCN-Daily Temperature and Precipitation Dataset (nClimGrid) \cite{nClimGrid1,nClimGrid2} of the United States National Oceanic and Atmospheric Administration (NOAA). The measurements are taken on a grid with spacing of $\frac{1}{24}$ of a degree for both latitude and longitude. We form an unweighted graph by connecting each measurement location to its eight neighbors on the grid, if they contain measurements, as shown in Fig. \ref{Fig:temp_graph_structure}. We then eliminate isolated vertices and small components (e.g., islands) to yield a connected graph with 469,404 vertices (measurement locations). Fig. \ref{Fig:temp_demo} shows the temperature measurements, an estimate of the cumulative spectral distribution function of the graph, and the resulting approximate filter bank with six bands and degree 80 polynomial approximations. Because of the fairly regular structure of this graph, the non-uniform sampling distributions look similar for each band. An intuitive explanation of the distribution for the first band, which is shown in Fig. \ref{Fig:temp_demo}(d)-(e), is that we want to sample with higher probability near the edges, as there are fewer neighbors from whom to interpolate the local average value. The 7,692 vertices selected for $\V_1$, shown in Fig. \ref{Fig:temp_demo}(f), are spread across the graph, with a higher density near the boundaries.

The magnitudes of the analysis coefficients, shown in Fig. \ref{Fig:temp_coeffs}, decay quickly, which is not surprising as the signal is generally smooth with some discontinuities that tend to coincide with topographical changes. We see in the second column of Fig. \ref{Fig:temp_reconstruction} that the non-zero analysis coefficients tend to cluster around these topographical changes, as expected. The first column of Fig. \ref{Fig:temp_reconstruction} shows an example atom at each band. If vertex $i \in \V_m$, then $\tilde{h}_m(\L){\boldsymbol \delta}_i$ is an atom that is localized within $K=80$ hops of node $i$ in the vertex domain. Its energy is also localized to the support of $\tilde{h}_m(\cdot)$ in the graph spectral domain. This joint vertex-frequency localization is a key property for wavelet-like transforms. 
...
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_atom1}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_filtered1}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_error1}}
\end{minipage} \\
\begin{minipage}[m]{0.15\linewidth}
\centerline{\small{Wavelet Scale 1~}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_atom2}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_filtered2}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_error2}}
\end{minipage} \\
\begin{minipage}[m]{0.15\linewidth}
\centerline{\small{Wavelet Scale 2~}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_atom3}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_filtered3}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_error3}}
\end{minipage} \\
\begin{minipage}[m]{0.15\linewidth}
\centerline{\small{Wavelet Scale 3~}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_atom4}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_filtered4}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_error4}}
\end{minipage} \\
\begin{minipage}[m]{0.15\linewidth}
\centerline{\small{Wavelet Scale 4~}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_atom5}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_filtered5}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_error5}}
\end{minipage} \\
\begin{minipage}[m]{0.15\linewidth}
\centerline{\small{Wavelet Scale 5~}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_atom6}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_filtered6}}
\end{minipage}
\begin{minipage}[m]{0.27\linewidth}
\centerline{\includegraphics[width=\linewidth]{fig_temp_error6}}
\end{minipage} 
\begin{minipage}[m]{0.15\linewidth}
